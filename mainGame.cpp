...
// Modul untuk menegecek kondisi draw untuk 7x7
int cekKosong7(char papan[7][7])
{
	int count = 0;
	for (int i = 0; i < 7; i++)
...
// Modul untuk memeriksa pemenang pada 5x5
int cekPemenang5(char papan[5][5])
{
	// Cek pemenang baris
...
int cekPemenang7(char papan[7][7])
{
	...
	{
		if ((papan[baris][0] == papan[baris][1] || papan[baris][5] == papan[baris][6] || papan[baris][1] == papan[baris][5]) && (papan[baris][1] == papan[baris][5]))
		{
			if (papan[baris][2] == 'X' && (papan[baris][0] == 'X' || papan[baris][4] == 'X'))
			{
				return 1;
